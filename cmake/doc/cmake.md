...
- `add_library(<name> [STATIC | SHARED | MODULE] [EXCLUDE_FROM_ALL] [source1] [source2 ...])` : ajout d'une librairie
- `add_executable(<name> [WIN32] [MACOSX_BUNDLE] [EXCLUDE_FROM_ALL] [source1] [source2 ...])` : ajout de la cible de la compilation avec `<name>` le nom de l'exécutable et `[sourceX]` les sources utilisées
